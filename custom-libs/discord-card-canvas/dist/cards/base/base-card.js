...
class BaseCardBuilder {
    ...
    constructor(params) {
        ...
        if (params.customLevelCard) {
            this.customLevelCard = params.customLevelCard
        }
    }
    /**
    ...
    async draw(ctx, canvasWidth, canvasHeight, options) {
        ...
        if (!options?.only || options.only.includes('mainText')) {
            // Main TextCard
            if (this.mainText) {
                textRender(this.mainText, 'main', 40, this.customLevelCard ? 240 : 225);
            }
        }
        if (!options?.only || options.only.includes('nickname')) {
            // Nickname
            if (this.nicknameText) {
                textRender(this.nicknameText, 'nickname', 60, this.customLevelCard ? 285 : 265);
            }
        }
