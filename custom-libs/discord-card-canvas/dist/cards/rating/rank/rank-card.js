...
exports.RankCardBuilder = void 0;
...
class RankCardBuilder {
    ...
    constructor(params) {
        ...
        if (params.avatarBackgroundColor)
            this.avatarBackgroundColor = params.avatarBackgroundColor;
        ...
        if (params.lvlPrefix)
            this.lvlPrefix = params.lvlPrefix;
        if (params.rankPrefix)
            this.rankPrefix = params.rankPrefix;
        if (params.progressBarColor)
            this.progressBarColor = params.progressBarColor;
        if (params.currentXPColor)
            this.currentXPColor = params.currentXPColor;
        if (params.requiredXPColor)
            this.requiredXPColor = params.requiredXPColor;
        if (params.avatarBackgroundEnable === false)
            this.avatarBackgroundEnable = false;
    }
    /**
    ...
    async draw(ctx, canvasWidth, canvasHeight, options) {
        if (!options?.only || options.only.includes('background')) {
            ...
            if (this.backgroundImgURL) {
                ...
                if (this.backgroundColor.bubbles) {
                    ...
                    ctx.fillStyle = (0, hex_rgba_1.hexToRgbA)(this.backgroundColor.bubbles, 0.31);
                    ctx.fill();
                    ctx.closePath();
                    ctx.arc(213, 81, 10, 0, Math.PI * 2);
                    ctx.fillStyle = (0, hex_rgba_1.hexToRgbA)(this.backgroundColor.bubbles, 0.07);
                    ctx.fill();
                    ctx.closePath();
                    ctx.arc(238, 16, 10, 0, Math.PI * 2);
                    ctx.fillStyle = (0, hex_rgba_1.hexToRgbA)(this.backgroundColor.bubbles, 0.6);
                    ctx.fill();
                    ctx.closePath();
                    ctx.beginPath();
                    ctx.arc(486, 148, 40, 0, Math.PI * 2);
                    ctx.fillStyle = (0, hex_rgba_1.hexToRgbA)(this.backgroundColor.bubbles, 0.1);
                    ctx.fill();
                    ctx.closePath();
                    ctx.beginPath();
                    ctx.arc(396.5, 33.5, 7.5, 0, Math.PI * 2);
                    ctx.fillStyle = (0, hex_rgba_1.hexToRgbA)(this.backgroundColor.bubbles, 0.05);
                    ctx.fill();
                    ctx.closePath();
                    ctx.beginPath();
                    ctx.arc(515.5, 38.5, 12.5, 0, Math.PI * 2);
                    ctx.fillStyle = (0, hex_rgba_1.hexToRgbA)(this.backgroundColor.bubbles, 0.43);
                    ctx.fill();
                    ctx.closePath();
                    ctx.beginPath();
                    ctx.arc(572, 257, 30, 0, Math.PI * 2);
                    ctx.fillStyle = (0, hex_rgba_1.hexToRgbA)(this.backgroundColor.bubbles, 1);
                    ctx.fill();
                    ctx.closePath();
                    ctx.beginPath();
                    ctx.arc(782.5, 226.5, 8.5, 0, Math.PI * 2);
                    ctx.fillStyle = (0, hex_rgba_1.hexToRgbA)(this.backgroundColor.bubbles, 0.15);
                    ctx.fill();
                    ctx.closePath();
                    ctx.beginPath();
                    ctx.arc(1000, 101, 10, 0, Math.PI * 2);
                    ctx.fillStyle = (0, hex_rgba_1.hexToRgbA)(this.backgroundColor.bubbles, 0.63);
                    ctx.fill();
                    ctx.closePath();
                }
            }
        ...
        if (!options?.only || options.only.includes('rank')) {
            ctx.textAlign = 'right';
            const rankFont = this.rankPrefix && this.rankPrefix.font ? this.rankPrefix.font : this.fontDefault;
            const rankContent = this.rankPrefix && this.rankPrefix.content ? this.rankPrefix.content : 'RANK';
            ctx.fillStyle =
        ...
        if (!options?.only || options.only.includes('lvl')) {
            // LVL
            const lvlFont = this.lvlPrefix && this.lvlPrefix.font ? this.lvlPrefix.font : this.fontDefault;
     ...
     */
    async build(options) {
        const canvas = (0, canvas_1.createCanvas)(1000, 250);
        const ctx = canvas.getContext('2d');
        await this.draw(ctx, canvas.width, canvas.height, options);
        return canvas;
    }
}
exports.RankCardBuilder = RankCardBuilder;
